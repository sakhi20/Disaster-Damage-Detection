...
class JointDamageNet(nn.Module):
    def __init__(self, loc_classes=1, dmg_classes=5, dropout=0.3):
        ...
        # Compress cat(pre, post, diff) → same size as cat(pre, post) so the
        # decoder sees a learned fusion rather than a redundant concatenation.
        self.dmg_reduce_high = nn.Sequential(
            nn.Conv2d(1024 * 3, 1024 * 2, 1, bias=False),
            nn.BatchNorm2d(1024 * 2), nn.ReLU()
        )
        self.dmg_reduce_low = nn.Sequential(
            nn.Conv2d(256 * 3, 256 * 2, 1, bias=False),
            nn.BatchNorm2d(256 * 2), nn.ReLU()
        )
    ...
    def forward(self, pre, post):
        ...
        loc_low  = torch.cat([pre_low,  post_low],  dim=1)
        loc_high = torch.cat([pre_high, post_high], dim=1)
        loc_out  = self.loc_head(loc_low, loc_high)

        # Damage: explicit change signal compressed back to 2× channel width
        low_diff  = post_low  - pre_low
        high_diff = post_high - pre_high

        dmg_high = self.dmg_reduce_high(torch.cat([pre_high, post_high, high_diff], dim=1))
        dmg_low  = self.dmg_reduce_low(torch.cat([pre_low,  post_low,  low_diff],  dim=1))
