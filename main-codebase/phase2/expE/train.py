...
def build_sample_weights(dataset):
    ...
    Minor damage (class 2) tiles are upsampled 6× — they were sharing a 3×
    bucket with major/destroyed in expD which diluted the minority signal.
    Major/destroyed tiles keep 3×; background-only tiles stay at 1×.
    """
    weights = []
    for _, _, _, post_mask_path, r, c in dataset.tiles:
        tile = np.array(Image.open(post_mask_path))[r:r + TILE_SIZE, c:c + TILE_SIZE]
        has_minor = np.any(tile == 2)
        has_major = np.any((tile == 3) | (tile == 4))
        if has_minor:
            weights.append(6.0)
        elif has_major:
            weights.append(3.0)
    ...
    with torch.set_grad_enabled(train):
        for pre, post, loc_mask, dmg_mask in pbar:
            ...
            if train:
                ...
                # unscale before clipping so the clip threshold is in true gradient space
...
def main():
    ...
    # T_0=15, T_mult=2: LR resets at epochs ~20 and ~50, allowing escape from the
    # plateau expD hit around epoch 23 with plain cosine decay.
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        if improved:
            ...
            for root in drive_roots:
                drive_dest = os.path.join(root, "ATML_project", "Phase-2 weights", "best_expE_model.pth")
